...
int main(void)
{
	for (int sym_pow = 16; sym_pow <= 18; sym_pow++)
	{
		...
		double Symbols = pow(2, sym_pow);							//Total Symbols
		double NumBits = Symbols * k;								//Total Number of Bits
		double NSpS = 4;											//Number of Samples per Symbol
		int N_spans = 10;											//Number of span divisions
		...
		for (int PchdBm_index = (int)Pch_dBm_min; PchdBm_index <= (int)Pch_dBm_max; PchdBm_index += 2)
		{
...
#		pragma omp parallel for num_threads(thread_count)
			...
			printf("\nPch =  %d       Symbols =  2^%d       BER =  %f\n\n", PchdBm_index, sym_pow, BER);
